    ...
    (testing "should contain a complete resources list"
      ...
      (testing "with all parameters"
        (is (= (query-to-vec ["SELECT cr.type, cr.title, rp.name, rp.value FROM catalog_resources cr, resource_params rp WHERE rp.resource=cr.resource ORDER BY cr.type, cr.title, rp.name"])
               [{:type "File" :title "/etc/foobar" :name "ensure" :value (sutils/munge-jsonb-for-storage "directory")}
                {:type "File" :title "/etc/foobar" :name "group" :value (sutils/munge-jsonb-for-storage "root")}
                {:type "File" :title "/etc/foobar" :name "user" :value (sutils/munge-jsonb-for-storage "root")}
                {:type "File" :title "/etc/foobar/baz" :name "ensure" :value (sutils/munge-jsonb-for-storage "directory")}
                {:type "File" :title "/etc/foobar/baz" :name "group" :value (sutils/munge-jsonb-for-storage "root")}
                {:type "File" :title "/etc/foobar/baz" :name "require" :value (sutils/munge-jsonb-for-storage "File[/etc/foobar]")}
                {:type "File" :title "/etc/foobar/baz" :name "user" :value (sutils/munge-jsonb-for-storage "root")}])))
